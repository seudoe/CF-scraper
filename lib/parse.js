...
export function parseBlocks(html) {
  ...
  while (remaining.length > 0) {
    ...
    // Skip unknown tags or plain text
...
export function parseProblemPage(html) {
  const psInner = extractInner(html, 'div', 'problem-statement');
  if (!psInner) { return null; }
  ...
  const inputRaw = (extractInner(psInner, 'div', 'input-specification') ?? '')
    .replace(/<div[^>]*class="[^"]*section-title[^"]*"[^>]*>[\s\S]*?<\/div>/i, '');
  const input = parseBlocks(inputRaw);

  const outputRaw = (extractInner(psInner, 'div', 'output-specification') ?? '')
    .replace(/<div[^>]*class="[^"]*section-title[^"]*"[^>]*>[\s\S]*?<\/div>/i, '');
  const output = parseBlocks(outputRaw);

  const examples = extractSampleTests(psInner);

  const noteRaw = (extractInner(psInner, 'div', 'note') ?? '')
    .replace(/<div[^>]*class="[^"]*section-title[^"]*"[^>]*>[\s\S]*?<\/div>/i, '');
  const noteBlocks = parseBlocks(noteRaw);
